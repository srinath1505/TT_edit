"use strict";
/**
 * Fills tools/i18n-overrides/bundles/amlPolicy.bodies.<locale>.json from
 * amlPolicy.bodies.en.json using google-translate-api-x.
 *
 * Usage: node tools/translate-aml-policy-google.cjs [vn|es-419|th|id|my|ph|pk|all]
...
function unmask(translated, tags) {
  ...
  for (let i = 0; i < tags.length; i++) {
    ...
    if (out.indexOf(ch) === -1) {
      console.warn("  placeholder lost for tag index", i);
    }
...
async function main() {
  ...
  if (arg !== "all" && TARGETS[arg]) locales = [arg];
    ...
    console.error(
      "Usage: node tools/translate-aml-policy-google.cjs [locale|all]"
    );
    process.exit(1);
  }

  for (const locale of locales) {
    console.log("Translating AML policy", locale, "→", TARGETS[locale]);
