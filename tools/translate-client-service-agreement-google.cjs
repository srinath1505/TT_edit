"use strict";
/**
 * Fills clientServiceAgreement.bodies.<locale>.json from .en.json via google-translate-api-x.
 * Usage: node tools/translate-client-service-agreement-google.cjs [vn|es-419|th|id|my|ph|pk|all]
 */
const fs = require("fs");
...
async function main() {
  ...
  if (arg !== "all" && TARGETS[arg]) locales = [arg];
    ...
    console.error(
      "Usage: node tools/translate-client-service-agreement-google.cjs [locale|all]"
    );
    process.exit(1);
  }
  ...
  for (const locale of locales) {
    console.log(
      "Translating client service agreement",
      locale,
      "→",
      TARGETS[locale]
    );
