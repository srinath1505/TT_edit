"use strict";
/**
 * Fills orderExecutionPolicy.bodies.<locale>.json from .en.json via google-translate-api-x.
 * Usage: node tools/translate-order-execution-policy-google.cjs [vn|es-419|th|id|my|ph|pk|all]
 */
const fs = require("fs");
...
async function main() {
  ...
  if (arg !== "all" && TARGETS[arg]) locales = [arg];
    ...
    console.error(
      "Usage: node tools/translate-order-execution-policy-google.cjs [locale|all]"
    );
    process.exit(1);
  }

  for (const locale of locales) {
    console.log("Translating order execution policy", locale, "→", TARGETS[locale]);
    ...
    const outPath = path.join(
      ...
      "i18n-overrides",
      "bundles",
      `orderExecutionPolicy.bodies.${locale}.json`
    );
