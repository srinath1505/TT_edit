...
(function() {
    ...
    var FLAGS = {
        ...
        singapore: '<svg viewBox="0 0 30 20" xmlns="http://www.w3.org/2000/svg"><rect fill="#ED2939" width="30" height="10"/><rect fill="#FFF" y="10" width="30" height="10"/><circle fill="#FFF" cx="6" cy="5" r="3.4"/><circle fill="#ED2939" cx="7.1" cy="5" r="2.7"/><polygon fill="#FFF" points="14.2,3.2 15.3,5.9 18.2,5.9 15.8,7.6 16.9,10.3 14.2,8.6 11.5,10.3 12.6,7.6 10.2,5.9 13.1,5.9"/></svg>',
    ...
    var REGION_GROUPS = [
        {
            titleKey: 'asiaPacific',
            titleFallback: 'Asia Pacific',
            countries: [
                { id: 'vietnam', nameFallback: 'Vietnam', offerFallback: 'Up to 100% Deposit Bonus' },
                { id: 'thailand', nameFallback: 'Thailand', offerFallback: '75% Bonus + Free Signals' },
                { id: 'malaysia', nameFallback: 'Malaysia', offerFallback: '100% Bonus + Zero Swap' },
                { id: 'philippines', nameFallback: 'Philippines', offerFallback: '100% Bonus + $10K Competition' },
                { id: 'indonesia', nameFallback: 'Indonesia', offerFallback: '50% Bonus + Islamic Account' },
                { id: 'singapore', nameFallback: 'Singapore', offerFallback: '100% Bonus + ECN Spreads' },
                { id: 'pakistan', nameFallback: 'Pakistan', offerFallback: '100% Bonus + Zero Swap' }
            ]
        },
        {
            titleKey: 'africa',
            titleFallback: 'Africa',
            countries: [
                { id: 'namibia', nameFallback: 'Namibia', offerFallback: '75% Bonus + Local Payment Extra' },
                { id: 'kenya', nameFallback: 'Kenya', offerFallback: '100% Bonus + M-Pesa 15% Extra' },
                { id: 'ghana', nameFallback: 'Ghana', offerFallback: '50% Bonus + 4% Cashback' },
                { id: 'nigeria', nameFallback: 'Nigeria', offerFallback: '100% Bonus + Naira Deposit Extra' },
                { id: 'south-africa', nameFallback: 'South Africa', offerFallback: '75% Bonus + ZAR Account' }
            ]
        },
        {
            titleKey: 'americas',
            titleFallback: 'Americas',
            countries: [
                { id: 'latam', nameFallback: 'Latin America', offerFallback: '100% Bono + $15K Championship' },
                { id: 'trinidad-tobago', nameFallback: 'Trinidad & Tobago', offerFallback: '100% Bonus + Loyalty Program' },
                { id: 'guyana', nameFallback: 'Guyana', offerFallback: '75% Bonus + Personal Manager' }
            ]
        }
    ];
    ...
    function regionName(id, fallback) {
        return t('offersPage.regions.' + id, fallback);
    }

    function regionSummary(id, fallback) {
        return t('offersPage.navDropdown.summaries.' + id, fallback);
    }

    function groupTitle(titleKey, fallback) {
        return t('offersPage.navDropdown.groups.' + titleKey, fallback);
    }

    function viewAllLabel() {
        return t('offersPage.navDropdown.viewAll', 'View All Offers →');
    }
    ...
    function buildDropdownHtml(basePath, lockedCountry, allCountriesDisabled) {
        ...
        REGION_GROUPS.forEach(function(group) {
            ...
            html +=
                '<h4 class="offers-dropdown-heading">' +
                escapeHtml(groupTitle(group.titleKey, group.titleFallback)) +
                '</h4>';
            group.countries.forEach(function(c) {
                ...
                var inner =
                    ...
                    escapeHtml(regionName(c.id, c.nameFallback)) +
                    '</span>' +
                    '<span class="offers-dropdown-offer">' +
                    escapeHtml(regionSummary(c.id, c.offerFallback)) +
                    '</span>' +
                    '</span>';
                if (isLockedInactive) {
                    html +=
        ...
        var viewAll = escapeHtml(viewAllLabel());
    ...
    window.addEventListener('languageChanged', renderOffersDropdowns);
    window.addEventListener('tradertok:i18n-applied', renderOffersDropdowns);
})();
